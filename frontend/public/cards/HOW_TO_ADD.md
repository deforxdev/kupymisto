...
5. Перезапусти API: `go run ./cmd/api` (або `docker compose up --build`).
...
Уже є мем-картинки (приклади):
- `owl-workout.webp` — сова на скакалці
- `lyashko-sosna.webp` — «кожному українцю сосну»
- `shcha-clock.webp` — годинник «ща / бля»
- `patron-route.webp`, `bavovna.webp`, `good-evening.webp`, `tractor-tank.webp` тощо
