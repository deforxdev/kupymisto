...
export default function GameScreen({room,user,onExit}:Props){
 ...
 const [selected,setSelected]=useState(0),[propertyOpen,setPropertyOpen]=useState(false),[pendingDeck,setPendingDeck]=useState<'chance'|'bad'|null>(null),[chance,setChance]=useState<ChanceEvent|null>(null),[casinoOpen,setCasinoOpen]=useState(false),[drawNonce,setDrawNonce]=useState(0),[tradeOpen,setTradeOpen]=useState(false),[adminOpen,setAdminOpen]=useState(false)
 ...
 const sameColorCells=current?.kind==='city'?cells.filter(cell=>cell.kind==='city'&&cell.color===current.color):[],ownsColorGroup=sameColorCells.length>0&&sameColorCells.every(cell=>{const index=cells.indexOf(cell);return liveRoom.ownership?.[String(index)]===user.id}),maxHouses=ownsColorGroup?5:3
 useEffect(()=>{
 ...
 const spinCasino=async()=>{const result=await api.casino(room.code);setLiveRoom(result.room);setCasinoOpen(false);setPhase('roll')}
 ...
 const buildHouse=async()=>{if(ownerId!==user.id||!current.price||balance<100||(liveRoom.houses?.[String(selected)]||0)>=maxHouses)return;setLiveRoom((await api.buildHouse(room.code,{cellIndex:selected})).room)}
