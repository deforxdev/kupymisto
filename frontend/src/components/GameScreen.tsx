...
export default function GameScreen({room,user,onExit}:Props){
 ...
 const [positions,setPositions]=useState(room.positions?.length===players.length?room.positions:players.map(()=>0)),[dice,setDice]=useState<[number,number]>(safeDice(room.dice)),[rolling,setRolling]=useState(false),[gameError,setGameError]=useState('')
