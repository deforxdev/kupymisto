...
import { ArrowLeftRight,Clock3,LogOut,Settings,Volume2,VolumeX } from 'lucide-react'
import { api,isAdminEmail,type Room,type User } from '../api'
import { isAudioMuted,playAssetSound,playDiceRoll,playPawnMove,playUiSound,setAudioMuted,unlockAudio } from '../audio'
...
export default function GameScreen({room,user,onExit}:Props){
 ...
 useEffect(()=>{const timer=window.setTimeout(()=>setShowStart(false),2200);void playAssetSound('game-start.ogg',()=>playUiSound('success'));return()=>window.clearTimeout(timer)},[])
 ...
 useEffect(()=>{
  ...
  void unlockAudio()
  void playAssetSound(delta>0?'money-in.ogg':'money-out.ogg',()=>playUiSound(delta>0?'success':'select'))
