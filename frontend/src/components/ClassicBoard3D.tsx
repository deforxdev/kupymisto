...
export function makeCells(size:BoardSize):BoardCell[]{const side=size==='large'?15:11,total=side*4-4;let city=0;return Array.from({length:total},(_,index)=>{if(index%(side-1)===0){const corners=['СТАРТ','Я У ПОЛЬЩІ','БУСИФІКАЦІЯ','DON’T PUSH THE HORSES'];return{name:corners[index/(side-1)],kind:'corner',color:index===0?'#e8bd32':'#d9e2d1'}}if(index%7===0)return{name:'ШАНС',kind:'chance',color:'#ded8c7'};if(index%9===0)return{name:'РЕМОНТ ДОРІГ',kind:'tax',color:'#d8d1bf'};if(index%5===0)return{name:'ВОКЗАЛ',kind:'station',price:200,color:'#d8d1bf'};const name=cityNames[city%cityNames.length],color=bands[Math.floor(city/2)%bands.length];city++;return{name,kind:'city',price:100+(city%9)*30,color}})}
...
function BoardModel({size,positions,players,dice,rolling,onSelectCell,ownership,drawNonce}:{size:BoardSize;positions:number[];players:Player[];dice:[number,number];rolling:boolean;onSelectCell:(index:number)=>void;ownership:Record<string,string>;drawNonce:number}){
